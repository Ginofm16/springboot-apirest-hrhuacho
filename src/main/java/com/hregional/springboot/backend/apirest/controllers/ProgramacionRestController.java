...
public class ProgramacionRestController {
	...
	public ResponseEntity<?> update(@Valid @RequestBody Programacion programacion, BindingResult result, @PathVariable Long id) {
		...
		programacionActual.setPro_hora_inicio(programacion.getPro_hora_inicio());
		programacionActual.setPro_sigla(programacion.getPro_sigla());
		programacionActual.setPro_num_turno(programacion.getPro_num_turno());
		programacionActual.setPersonal(programacion.getPersonal());
	...
	@GetMapping("/programacion/consul/{cod}")
	public List<Programacion> findAllProgramacionByConsultorio(@PathVariable Long cod){
		return programacionService.findAllProgramacionByConsultorio(cod);
	}
